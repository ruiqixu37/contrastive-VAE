...
class Autoencoder(nn.Module):
    ...
    def calc_bce_loss(self, xbin_ND):
        ...
        xproba_ND = self.forward(xbin_ND)               # <-- TODO fix me
        
        bce = F.binary_cross_entropy(xproba_ND, xbin_ND, reduction='sum') \
            / xproba_ND.size(dim=0) \
            / xproba_ND.size(dim=1)  # <-- TODO fix me
        if torch.isnan(bce).any():
            print('xproba_ND', xproba_ND)
            print('xbin_ND', xbin_ND)
            print('bce', bce)
            raise ValueError('NaN in BCE loss')
